...
//      let catalog = catalogSection.querySelector('.catalog__main');
//      let catalogNav = catalogSection.querySelector('.filter-catalog');
//      let catalogItem = catalogSection.querySelectorAll('.product__item');
   
//      catalogNav.addEventListener('click', function (e) {
//         let target = e.target;
//         console.log(target)
//         var item = myLib.closestItemByClass(target, 'filter-catalog__item');

//         if(item === null || item.classList.contains('_active')) {
//            return;
//         }
         
//         e.preventDefault();
//         let filterValue = item.getAttribute('data-filter');
//         let previousBtnActive = catalogNav.querySelector('.filter-catalog__item._active');
//         previousBtnActive.classList.remove('_active');
...
let slider_about = new Swiper('.sliders-gallery__body', {
   slidesPerView: 3,
   centeredSlides: true,
   initialSlide: 1,
   observer: true,
   ...
   parallax: true,
   navigation: {
      nextEl: '.sliders-gallery .slider-arrow_next',
      prevEl: '.sliders-gallery .slider-arrow_prev',
   },
...
;(function() {
   ...
   var catalog = catalogSection.querySelector('.catalog__main');
   var catalogNav = catalogSection.querySelector('.filter-catalog');
   var catalogItems = catalogSection.querySelectorAll('.product__item');
   ...
   catalogNav.addEventListener('click', function(e) {
     ...
     var previousBtnActive = catalogNav.querySelector('.filter-catalog__item._active');
...
const cartTotalCounterDOMElement = document.querySelector('.js-cart-total-count-items');
...
const cartWrapperDOMElement = document.querySelector('.cart');

const renderCartItem = ({id, name, attribute, size, src, price, quantity}) => {
const cartItemDOMElement = document.createElement('div');
const attributeTemplateType = attribute
? `<div class="products-cart__attribute">${attribute}</div><input type="hidden" name="${id} - Атрибут" value="${attribute}">` 
: '';
const attributeTemplateSize = size 
? `<div class="products-cart__size">${size}</div><input type="hidden" name="${id} - Атрибут" value="${size}">` 
: '';
   ...
   const cartItemTemplate = `
      <div class="products-cart__product">
            ...
            <div class="products-cart__description">
               <a class="products-cart__title">	${name}</a>
               ...
               ${attributeTemplateType}
               ${attributeTemplateSize}
            </div>
            <div class="products-cart__quantity">
               <div class="quantity">
                  <div class="quantity__button quantity__button_minus"></div>
                  <span class="quantity__item js-cart-item-quantity">${quantity}</span>
                  <div class="quantity__button quantity__button_plus"></div>
               </div>
            </div>
            <div class="products-cart__price"><span class="js-cart-item-price">${price * quantity}</span>грн</div>
            <div class="products-cart__del _icon-plus js-btn-cart-item-remove"></div>
      </div>
   `;
...
const updateCartTotalCounter = () => {
   const totalCounter = Object.keys(cart).reduce((acc, id) => {
      const {quantity} = cart[id];
      return acc + quantity;
   }, 0);
   
      if(cartTotalCounterDOMElement) {
         cartTotalCounterDOMElement.textContent = totalCounter;
      };

   };

const updateCart = () => {
   const totalQuantity = updateCartTotalPrice();
   updateCartTotalCounter();
   saveCart();

   if(totalQuantity === 0) {
      cartWrapperDOMElement.classList.remove('_active');
   } else {
      cartWrapperDOMElement.classList.add('_active');
   }
};
...
const getProductData = (productDOMElement) => {
   ...
   const quantity = 1;
   const id = generateID(name, attribute, size);
   
   return {name, attribute, size, price, src, quantity, id};
};
...
const generateID = (string1, string2, string3) => {
   const secondParam = string2 && string3 ? `-${string2}, ${string3}` : '';
   return `${string1}${secondParam}`
};

const flyToCart = (productElem) => {
   const addToCartBtn = document.querySelector('.js-btn-add-to-cart');
   if(!addToCartBtn.classList.contains('_fly')){
      addToCartBtn.classList.remove('_fly');
      addToCartBtn.classList.add('_fly');

      const cartIcon = document.querySelector('.cart__icon-wrap');
      
      const itemProduct = productElem;
      const productImage = itemProduct.querySelector('.products__image');
...
const CartInit = () => {
   ...
   document.querySelector('body').addEventListener('click', (e) => {
      ...
      if (target.closest('.options-products__size')) {
         const optionPrice = target.closest('.select__option');
         const price = optionPrice.getAttribute('data-value');
         const size = optionPrice.textContent;
         ...
         productDOMElement.setAttribute('data-product-size', size);
      }

      if (target.closest('.options-products__type')) {
         const optionAttribute = target.closest('.select__option');
         const attribute = optionAttribute.getAttribute('data-value');
